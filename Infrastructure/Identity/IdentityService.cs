    ...
    public class IdentityService : IIdentityService
    {
        ...
        public async Task<Result> Login(string email, string password)
        {
            ...
            if (user == null || !await _userManager.CheckPasswordAsync(user, password))
                return Result.Faliure("Invalid username or password");

            if(!user.EmailConfirmed)
                return Result.Faliure("E-mail not confirmed");

            var claims = GetClaims(user);
            var token =  GenerateJwtForUser(user, claims);

            return Result.Successfull(token);
        }
        ...
        public async Task<Result> ConfirmEmail(string email, string token)
        {
            ...
            if (user == null)
                return Result.Faliure("There was an error while processing confirm email the request");
            ...
            if (emailConfirmed.Succeeded)
                ...
                return Result.Faliure("There was error during confirmation");
        }
        ...
        public async Task<Result> ResetPassword(string email, string token, string password)
        {
            ...
            if (user == null)
                return Result.Faliure("There was an error while processing reset password");
            ...
            if (resetPasswordResult.Succeeded)
                ...
                return Result.Faliure("There was error during password reset request");
        }
        ...
        public async Task<Result> GenerateTokenForIdentityPurpose(string email, TokenPurpose purpose)
        {
            ...
            if (user == null)
                return Result.Faliure("There was an error while processing token request");
